...
## 系统风格三窗红框差异排查（2026-09-17 用户青绿截图 8 处红框定案）
...
- **背景图素材与动态元素分离**：商品卡上的**加购按钮**（主色圆+白加号，icon-jiarugouwuche）、W2 **邀请区圆形人物头像组**、W3 **上门自提浅青标签**均为菜鸟云**动态覆盖元素**（不在 PNG 素材内）——定位必须查对标 DOM（`.choose_*` 类名/rect），素材裁剪区为空白/原文字即证明是动态层。
- **层级规则补充**：覆盖层在背景图**之下**（z0），但**盖在背景图之上的动态元素**（加购按钮、头像组）必须独立 `z-index: 3`，不能放进 z0 覆盖层（会被背景图盖住）。
- **上门自提标签**：浅青背景圆角矩形 = `rgba(primaryColor, 0.25)` + `border-radius 12px` + 主色文字（菜鸟云为 iconfont 字形，文字色 primaryColor）；背景随主题变色（青绿→浅青、玫红→浅玫红）。
- **1:1 位置换算**：用户截图的元素坐标（原图像素）→ 窗内坐标按缩放比换算（窗 250×466 → 截图窗宽/高比例），落位后对照 DOM getBoundingClientRect 复核；加购按钮窗内 (308,218) 与菜鸟云一致即达标。
