...
- **颜色实测色值（菜鸟云 stylediy 用户图 4 红框定案 + 用户三轮配色角色纠正 + 上门自提 tab 纠正）**：上门自提 = **浅青圆角矩形选中 tab（`rgba(primary,0.25)` 底 + 主色文字居中）**，占窗右半（`left:50%;width:50%;height:40px;flex center`），与素材「快递配送」左半 tab 同行并排（**不是纯文字 right 浮动**——用户纠正"没有在选项卡居中"，裁 8 倍放大菜鸟云图确认 tab 有浅青圆角底）；加入购物车 = **辅助颜色 `secondaryColor` 实色底 + 文字辅色 `subTextColor` 字**（非半透明浅青，用户纠正"辅助颜色"）；立即购买 = **主题颜色 `primaryColor` 纯色底 + 白字**（非渐变，用户纠正"主题颜色"）；邀请胶囊 = **主题色→渐变 `linear-gradient(90deg, primaryColor, gradientColor)` 实色渐变底 + 白字**（非浅青半透明渐变，用户纠正"主题颜色和渐变颜色组成的"）；提交订单 = 主色实底 + 白字圆角；已售300份 = **纯主色文字 + 主色火焰图标在文字上方（flex column）**；标签行 = 白底 + 主色浅青描边 + 主色文字。
