...
- **颜色实测色值（菜鸟云 stylediy 用户图 4 红框定案）**：上门自提 = **纯主色文字（无底无边框）**；加入购物车 = `rgba(primaryColor,0.25)` 浅青底 + 主色文字；立即购买 = 主色渐变底 + 白字；提交订单 = 主色实底 + 白字圆角；已售300份 = **纯主色文字 + 主色火焰图标在文字上方（flex column）**；邀请胶囊 = 浅青渐变底 + 主色字；标签行 = 白底 + 主色浅青描边 + 主色文字。
