...
- **应用级菜单门禁（2026-09-17 新增，漏菜单三次教训沉淀）**：凡复刻一个**应用**（如卡密库/礼品卡券/送礼物/商品/会员等，含侧边栏或页内导航的多页面模块），交付前必须通过三道门禁，任一不过不得提交：
  ① **全量抓菜单**：进入该应用在菜鸟云的第一页后，先用浏览器把**完整侧边栏/导航链接全量抓出**（含所有子菜单及其 URL），形成「菜单核对清单」——禁止凭印象/凭入口页判断菜单数量；
  ② **菜单数对齐**：我方应用页内 Tab 数必须 ≥ 对标菜单数（对标每个菜单项在我方都必须有对应页面或 Tab），数量不一致直接判定漏项，先补后交付；
  ③ **逐菜单实测**：清单上**每一个菜单项都必须实际点开**，抓取该页字段（列表列/表单字段/设置项/弹窗），禁止只测主列表页就交付——「基础设置」「订单」这类非列表页最易漏。
  此门禁与「逐项点击核对」并行：逐项点击覆盖页内控件，菜单门禁覆盖页面层级；**漏任何一层都视为未执行规范**。
