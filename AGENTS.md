...
9. **字段全集以实测为准，禁止想当然增删字段**：复刻字段必须全部来自对标面板实测滚读结果，**不得凭业务直觉新增字段**（教训：曾给倒计时02 误加「主图比例」三选项图形块——eweishop 面板根本没有该字段，那 3 个带尺寸的图形只是图片字段三图占位区的标注，被当成独立字段误加）。凡是「我以为应该有」的字段，先回对标面板找证据：找不到就不加，找到再按实际形态复刻。
