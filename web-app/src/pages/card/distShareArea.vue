<template>
  <view class="dist-page">
    ...
    <view class="hero g3" :style="heroStyle">
      <view class="hero-top">
        <view class="hero-title">区域股东</view>
      ...
      <view class="user-row">
        ...
        <!-- 地区股东标签组 -->
        <view v-if="areaTags.length" class="tag-row">
          <view v-for="t in areaTags" :key="t" class="tag">{{ t }}</view>
        </view>
      </view>
    ...
    <!-- 无身份提示 -->
    <view v-if="!areaTags.length" class="tip-box">
      ...
      <view class="qr-panel rules-panel" @click.stop>
        ...
        <scroll-view scroll-y class="rules-scroll">
          <view class="rules-text">
            <view class="rule-p">· 区域股东按「订单买家名片归属地区」触发对应地区分红池</view>
            <view class="rule-p">· 分红金额 = 订单实付金额 × 该地区分红比例 × 权重占比</view>
            <view class="rule-p">· 订单售后退款会扣回对应分红</view>
            <view class="rule-p">· 后台撤销区域股东身份后，历史收益保留、不再产生新分红</view>
          </view>
        </scroll-view>
...
onShow(() => {
  trackPageView('distribution_share_area');
  loadAll();
});
</script>
...
.log-src { font-size: 22rpx; color: #14b8a6; background: #ecfdf9; padding: 2rpx 12rpx; border-radius: 999rpx; }
