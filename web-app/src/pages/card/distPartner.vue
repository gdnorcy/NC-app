<template>
  <view class="dist-page">
    ...
    <view class="hero g3" :style="heroStyle">
      <view class="hero-top">
        <view class="hero-title">合伙人分红</view>
      ...
      <view class="user-row">
        <view class="user-line">
          ...
          <view class="user-level">{{ partnerModeLabel }}</view>
        </view>
      </view>
      <view class="money-row">
        <view class="money-left">
          <view class="money-label">待分红（元）</view>
          <view class="money-val">{{ fen(summary.partnerPending) }}</view>
        </view>
        <view class="money-right">
          <view class="money-right-item"><text class="mr-lb">累计分红</text><text class="mr-val">{{ fen(summary.partnerTotal) }}</text></view>
        </view>
      </view>
    </view>

    <!-- 无身份提示 -->
    <view v-if="!summary.isPartner" class="tip-box">
      <text v-if="summary.unbound">当前暂无分红权限，入驻租户后可查看收益</text>
      <text v-else>暂未获得合伙人身份，可以向租户管理员申请开通</text>
    </view>

    <!-- 统计卡 -->
    <view v-if="summary.isPartner" class="stat-cards">
      ...
      <view class="qr-panel rules-panel" @click.stop>
        ...
        <scroll-view scroll-y class="rules-scroll">
          <view class="rules-text">
            <view class="rule-p">· 合伙人对团队/租户全局订单流水按权重参与分红</view>
            <view class="rule-p">· 分红金额 = 订单实付金额 × 分红池比例 × 权重占比</view>
            <view class="rule-p">· 订单售后退款会扣回对应分红</view>
            <view class="rule-p">· 后台移除合伙人身份后，历史收益保留、不再产生新分红</view>
          </view>
        </scroll-view>
...
async function loadAll() {
  try {
    ...
    summary.value = { wallet: null, selfName: '我', selfAvatar: '', unbound: true, isPartner: false, partnerPending: 0, partnerTotal: 0, shareTags: [] };
  }
  try {
...
onShow(() => {
  trackPageView('distribution_partner');
  loadAll();
});
</script>
...
.stat-cards { margin: 24rpx 32rpx; display: grid; grid-template-columns: repeat(4, 1fr); gap: 16rpx; }
