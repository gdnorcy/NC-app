...
import buyBtn1 from '../../../../assets/design-thumbs/buyBtn1.png';
import buyBtn2 from '../../../../assets/design-thumbs/buyBtn2.png';
import buyBtn3 from '../../../../assets/design-thumbs/buyBtn3.png';
...
const schemaSections = computed(() => {
  ...
  if (grpFields.length) {
    ...
    // 每个section分noWhenFields（无when条件）和whenFields（有when条件），whenFields包灰底
    merged.forEach(sec => {
      sec.noWhenFields = sec.fields.filter(f => !f.when);
      sec.whenFields = sec.fields.filter(f => f.when);
    });
    return merged;
  }
  const result = buildSecs(def, whenOk, common);
  result.forEach(sec => {
    sec.noWhenFields = sec.fields.filter(f => !f.when);
    sec.whenFields = sec.fields.filter(f => f.when);
  });
...
.pe-lib-ico img { width: 36px; height: 36px; object-fit: contain; display: block; }
...
.pe-prop-body :deep(.el-form-item.pe-when-field) { background: #f7f8fa; margin: 0 !important; margin-bottom: 0 !important; margin-top: 0 !important; padding: 8px 10px !important; }
.pe-prop-body :deep(.el-form-item.pe-when-field + .el-form-item.pe-when-field) { margin-top: 0 !important; padding-top: 0 !important; }
...
.el-radio-group { display:flex; flex-wrap:nowrap; align-items:center; gap:0; width:100%; }
...
.pe-label-top .el-form-item__label-wrap { display:block !important; margin-left:0 !important; }
.pe-label-top .el-form-item__label { display:block !important; width:auto !important; text-align:left !important; justify-content:flex-start !important; padding-bottom:8px !important; float:none !important; }
.pe-label-top .el-form-item__content { margin-left:0 !important; }
